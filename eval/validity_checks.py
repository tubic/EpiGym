#!/usr/bin/env python3
"""Compute descriptive validation summaries from the bundled EpiGym files. Run:
...
def check_contact_gradient():
    """Summarize measured |ε| by Cβ-Cβ distance bin."""
    d = _load("data/tsuboyama_stability_doubles.csv").dropna(
        subset=["cb_distance", "epistasis_z"]
    )
...
def check_contact_vs_distant():
    """Compare contact and distant T2 correlations for each reference model."""
    perd = _load("baselines/tsuboyama_epistasis_predictions_per_double.csv")
    sc = _load("data/tsuboyama_stability_doubles.csv")[
        ["DMS_id", "mutant", "score", "additive_pred"]
    ]
    ...
    for dom, g in perd.groupby("DMS_id"):
        slo, shi = g.score.quantile([0.15, 0.85])
        alo, ahi = g.additive_pred.quantile([0.15, 0.85])
        keep.append(
            g[
                (g.score > slo)
                & (g.score < shi)
...
def check_epistasis_type():
    """Summarize SKEMPI epistasis classes and mean absolute residuals."""
    ...
    for ty in order:
        g = t[t.type == ty]
        print(
            f"    {ty:16s} {len(g) / len(t) * 100:6.1f}% {g.epistasis.abs().mean():19.2f}"
        )
    qual = t.type.isin(["sign", "reciprocal-sign"]).mean()
    print(f"    sign + reciprocal-sign share: {qual * 100:.0f}%")


def check_cross_assay():
    """Summarize selected Tsuboyama and MAAD reference metrics."""
    ...
    print(f"    Tsuboyama: highest median |ρ|={best.median_abs_rho:.2f} ({best.model})")
    ml = _load("baselines/maad_escape_leaderboard.csv")
    rec = ml[
        ml.split.isin(
            ["random", "seen_positions_unseen_combinations", "ba12_to_xbb_bq"]
        )
    ].auroc_gain.mean()
    nov = ml[
        ml.split.isin(
            ["low_to_high_burden", "pre_omicron_to_omicron", "cross_virus_hardest"]
        )
    ].auroc_gain.mean()
    print(
        f"    escape (MAAD): interaction AUROC gain = {rec:+.02f} on recombination splits vs {nov:+.02f} on novel-mutation splits"
    )
    print("    values are read from the bundled reference leaderboards")


if __name__ == "__main__":
    print("EpiGym validation summaries")
    check_contact_gradient()
    check_contact_vs_distant()
    check_epistasis_type()
    check_cross_assay()
    print("\nValidation summaries complete.")
